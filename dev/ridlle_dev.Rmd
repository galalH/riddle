...
#' @rdname resource
#' @export
resource_update <- function(id, metadata, uat) {
  enc <- if(is.null(metadata$upload)) "json" else "multipart"
...
#' @rdname resource
#' @export
resource_patch <- function(id, metadata, uat) {
  enc <- if(is.null(metadata$upload)) "json" else "multipart"
  ridl("resource_patch",
       id = id,
       !!!metadata, 
...
## let's get again the details of the dataset we want to add the resource in..
r <- riddle::resource_create(ridlid, m, uat = TRUE)
