...
dataset_create <- function(metadata,
                           uat= FALSE) { 
  ridl("package_create", !!!metadata, uat) %>% 
    dataset_tibblify() }
...
dataset_update <- function(id, metadata, 
                           uat= FALSE) { 
  ridl("package_update", id = id, !!!metadata, uat) %>% 
    dataset_tibblify() }

#' @rdname dataset
#' @export
dataset_patch <- function(id, metadata, 
                          uat= FALSE) { 
  ridl("package_patch", id = id, !!!metadata, uat) %>% 
    dataset_tibblify() }
...
dataset_search <- function(q = NULL, rows = NULL, start = NULL) {
  r <- ridl("package_search", !!!(as.list(match.call()[-1])) , uat)$results
...
#'  not explicitly provided in the `metadata`.
#'
#' @param metadatares Metadata created by \code{\link{resource_metadata()}}.
...
#' @return The resource.
#' @export
resource_create <- function(pkgid, metadatares, uat) {
  enc <- if(is.null(metadatares$upload)) "json" else "multipart"
  ridl("resource_create", 
       dataset_id = pkgid, !!!metadatares,
       .encoding = enc,
       uat) %>% 
    resource_tibblify()
}
...
#' @rdname resource
#' @export
resource_update <- function(id, metadatares, uat) {
  enc <- if(is.null(metadatares$upload)) "json" else "multipart"
  ridl("resource_update",
       id = id,
       !!!metadatares,
       .encoding = enc,
       uat) %>% 
...
#' @rdname resource
#' @export
resource_patch <- function(id, metadatares, uat) {
  enc <- if(is.null(metadatares$upload)) "json" else "multipart"
  ridl("resource_patch",
       id = id,
       !!!metadatares, 
...
## let's get again the details of the dataset we want to add the resource in..
r <- riddle::resource_create(ridlid, metadatares = m, uat = TRUE)
